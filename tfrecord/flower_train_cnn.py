...
def flower_input(if_random = True, if_training = True):
    ...
    filename_queue = tf.train.string_input_producer(filenames) # 将filenames输出到输入管道的队列
...
def flower_train():
    ...
    label_batch_one_hot = tf.one_hot(tf.add(label_batch_out, label_offset), depth=5, on_value=1.0, off_value=0.0) #偏移offset，从1开始
