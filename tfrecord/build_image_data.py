...
tf.app.flags.DEFINE_integer('train_shards', 2,
                            'Number of shards in training TFRecord files.') # tfrecord拆成几个文件存储
tf.app.flags.DEFINE_integer('validation_shards', 2,
                            'Number of shards in validation TFRecord files.')

tf.app.flags.DEFINE_integer('num_threads', 2,
                            'Number of threads to preprocess the images.') # 启动几个线程
...
tf.app.flags.DEFINE_string('labels_file', './flower_label.txt', 'Labels file') # label文件指定label
...
def _process_image_files(name, filenames, texts, labels, num_shards):
  ...
  sys.stdout.flush()   # 刷新输出 用于多线程

  # Create a mechanism for monitoring when all threads are finished.
  coord = tf.train.Coordinator()  # TensorFlow线程管理器
...
if __name__ == '__main__':
  tf.app.run()  # 通过处理flag解析，然后执行main函数。
